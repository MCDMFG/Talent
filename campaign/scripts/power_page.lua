...
function onInit()
	...
	DB.addHandler(DB.getPath(node, "powergroup.*.powerdie"), "onUpdate", onAbilityChanged);
...
function onClose()
	...
	DB.removeHandler(DB.getPath(node, "powergroup.*.powerdie"), "onUpdate", onAbilityChanged);
...
function updatePowerWindowUses(nodeChar, w)
	...
	local bCaster = (rGroup and rGroup.grouptype == "memorization");
	local bPsionics = (rGroup and rGroup.grouptype == "psionic");
